...
CStaticObj::CStaticObj(LPDIRECT3DDEVICE9 pDevice)
...
HRESULT CStaticObj::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_StaticObject");
...
CStaticObj* CStaticObj::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CStaticObj*		pGameObject = new CStaticObj(pDevice);
...
HRESULT CStaticObj::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
