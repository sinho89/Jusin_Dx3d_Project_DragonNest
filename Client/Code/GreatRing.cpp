...
CGreatRing::CGreatRing(LPDIRECT3DDEVICE9 pDevice)
...
CGreatRing::~CGreatRing(void)
{

}

HRESULT CGreatRing::Initialize(void)
{
...
HRESULT CGreatRing::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_GreatRing");
...
CGreatRing* CGreatRing::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CGreatRing*		pGameObject = new CGreatRing(pDevice);
...
CGreatRing* CGreatRing::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CGreatRing*		pGameObject = new CGreatRing(pDevice);
...
HRESULT CGreatRing::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CGreatRing::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CGreatRing::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
