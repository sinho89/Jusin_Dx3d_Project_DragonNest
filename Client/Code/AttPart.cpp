...
CAttPart::CAttPart(LPDIRECT3DDEVICE9 pDevice)
: CParticle(pDevice)
...
CAttPart* CAttPart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CAttPart*		pPetal = new CAttPart(pDevice);
...
CAttPart* CAttPart::Create(LPDIRECT3DDEVICE9 pDevice, D3DXVECTOR3 _vPos)
{
	CAttPart*		pPetal = new CAttPart(pDevice);
	...
	pPetal->SetInfo(_vPos);

	return pPetal;
}
void CAttPart::SetInfo(D3DXVECTOR3 _vPos)
{
	m_pInfo->m_vPos = _vPos;
}


HRESULT CAttPart::SetConstantTable(void)
{
...
HRESULT CAttPart::AddComponent(void)
{
...
HRESULT CAttPart::Initialize(void)
{
	AddComponent();
...
void CAttPart::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CAttPart::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CAttPart::SetPart(int nIdx)
{
	...
	// 초기 가속도
	pPrt->m_IntA = D3DXVECTOR3(0.f, 1.f, 0.f) * 10.f;
	...
	m_pInfo->m_vDir = D3DXVECTOR3(0.f, 1.f, 0.f);
	...
	pPrt->m_fFade	=( 100 + rand()%101  ) *0.5f;

	pPrt->m_dColor	=	D3DXCOLOR(0.25f, 1.f, 0.f, pPrt->m_fLife);

	// 입자의 표현 요소
	pPrt->m_PrsW = 10000.f + rand()%61;
	pPrt->m_PrsW *= 0.03f;

	pPrt->m_PrsH = 10000.f + rand()%61;
	pPrt->m_PrsH *= 0.03f;
