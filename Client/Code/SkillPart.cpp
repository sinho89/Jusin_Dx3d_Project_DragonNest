...
CSkillPart::CSkillPart(LPDIRECT3DDEVICE9 pDevice)
: CParticle(pDevice)
...
CSkillPart::~CSkillPart()
{

}
CSkillPart* CSkillPart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CSkillPart*		pPetal = new CSkillPart(pDevice);
...
HRESULT CSkillPart::SetConstantTable(void)
{
...
HRESULT CSkillPart::AddComponent(void)
{
...
HRESULT CSkillPart::Initialize(void)
{
	AddComponent();
...
void CSkillPart::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CSkillPart::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CSkillPart::SetPart(int nIdx)
{
	...
	pPrt->m_fFade	=( 100 + rand()%101  ) *0.01f;
	pPrt->m_dColor	=	D3DXCOLOR(1.f, 1.f, 1.f, 1.f);
