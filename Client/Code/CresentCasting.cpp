...
CCresentCasting::CCresentCasting(LPDIRECT3DDEVICE9 pDevice)
...
CCresentCasting::~CCresentCasting(void)
{

}

HRESULT CCresentCasting::Initialize(void)
{
...
void CCresentCasting::Update(void)
{
...
void CCresentCasting::Render(void)
{
	if(m_pInfo->m_bSkillRender)
	{
		...
		m_pMesh->RenderMesh_ForSDK(pEffect, &m_pInfo->m_matWorld, Engine::OBJ_ALPHA);

		m_pDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);
	}
}

HRESULT CCresentCasting::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_CresentCasting");
...
CCresentCasting* CCresentCasting::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CCresentCasting*		pGameObject = new CCresentCasting(pDevice);
...
CCresentCasting* CCresentCasting::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CCresentCasting*		pGameObject = new CCresentCasting(pDevice);
...
HRESULT CCresentCasting::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CCresentCasting::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CCresentCasting::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
