...
CHarfMoon::CHarfMoon(LPDIRECT3DDEVICE9 pDevice)
...
CHarfMoon::~CHarfMoon(void)
{

}

HRESULT CHarfMoon::Initialize(void)
{
	FAILED_CHECK(AddComponent());
	m_fDetail = 1.f;
	m_pCollisionMgr = CCollisionMgr::GetInstance();
	m_pInfo->m_vPos = D3DXVECTOR3(99999.f, 99999.f, 99999.f);

	return S_OK;
}


void CHarfMoon::Update(void)
{
	...
	if(m_pInfo->m_bSkillRender)
	{
		...
		m_pInfo->m_vPos += m_pInfo->m_vDir * 1200.f * fTime;
		m_pInfo->m_vScale = D3DXVECTOR3(m_fScale, 1.5f, m_fScale );
		m_fScale += 0.5f;
		m_fAlpha += 0.1f;
	}
	else
...
HRESULT CHarfMoon::AddComponent(void)
{
...
CHarfMoon* CHarfMoon::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHarfMoon*		pGameObject = new CHarfMoon(pDevice);
...
CHarfMoon* CHarfMoon::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHarfMoon*		pGameObject = new CHarfMoon(pDevice);
...
HRESULT CHarfMoon::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CHarfMoon::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHarfMoon::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}

void CHarfMoon::Collision_OBB(void)
{
	...
	float fDistance = D3DXVec3Length(&(pBossInfo->m_vPos - m_pInfo->m_vPos));

	if(fDistance < 100)
	{
		if(!pBossInfo->m_bSkillHit)
		{
