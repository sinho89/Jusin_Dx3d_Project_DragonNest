...
CTrailPart::CTrailPart(LPDIRECT3DDEVICE9 pDevice)
: CParticle(pDevice)
...
CTrailPart::~CTrailPart()
{

}
CTrailPart* CTrailPart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CTrailPart*		pPetal = new CTrailPart(pDevice);
...
HRESULT CTrailPart::SetConstantTable(void)
{
...
HRESULT CTrailPart::AddComponent(void)
{
...
HRESULT CTrailPart::Initialize(void)
{
	AddComponent();

	m_pInfo->m_vPos = D3DXVECTOR3(999999.f, 999999.f, 999999.f);
	m_pInfo->m_eColorType = Engine::COLOR_BLUE;
	m_PrtN	= 500;
...
void CTrailPart::Update(void)
{
		...
		switch(m_pInfo->m_eColorType)
		{
		case Engine::COLOR_BLUE:
			for(int i=0; i<m_PrtN; ++i)
			{
				Tpart* pPrt = &m_PrtD[i];
		...
		case Engine::COLOR_RED:
			...
			break;
		case Engine::COLOR_PURPLE:
			for(int i=0; i<m_PrtN; ++i)
			{
				Tpart* pPrt = &m_PrtD[i];
				pPrt->m_dColor	=	D3DXCOLOR(1.f, 0.25f, 1.f, pPrt->m_fLife);
			}
			break;
		}
...
void CTrailPart::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CTrailPart::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CTrailPart::SetPart(int nIdx)
{
	...
	m_pInfo->m_vDir.x = m_pInfo->m_vDir.x + (rand() % 4 - 2) * 0.1;
	m_pInfo->m_vDir.y = m_pInfo->m_vDir.y + (rand() % 4 - 2) * 0.1;
	m_pInfo->m_vDir.z = m_pInfo->m_vDir.z + (rand() % 4 - 2) * 0.1;
