...
CGreatAura::CGreatAura(LPDIRECT3DDEVICE9 pDevice)
...
CGreatAura::~CGreatAura(void)
{

}

HRESULT CGreatAura::Initialize(void)
{
...
HRESULT CGreatAura::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_GreatAura");
...
CGreatAura* CGreatAura::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CGreatAura*		pGameObject = new CGreatAura(pDevice);
...
CGreatAura* CGreatAura::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CGreatAura*		pGameObject = new CGreatAura(pDevice);
...
HRESULT CGreatAura::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CGreatAura::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CGreatAura::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
