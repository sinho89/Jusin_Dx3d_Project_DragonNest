...
CCycle::CCycle(LPDIRECT3DDEVICE9 pDevice)
...
HRESULT CCycle::Initialize(void)
{
...
void CCycle::Update(void)
{
	...
	{
		m_pInfo->m_fAngle[Engine::ANGLE_Y] += D3DXToRadian(2.f);
...
HRESULT CCycle::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_Cycle");
...
CCycle* CCycle::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CCycle*		pGameObject = new CCycle(pDevice);
...
CCycle* CCycle::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CCycle*		pGameObject = new CCycle(pDevice);
...
HRESULT CCycle::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CCycle::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CCycle::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
