...
CGreatPart::CGreatPart(LPDIRECT3DDEVICE9 pDevice)
: CParticle(pDevice)
...
CGreatPart::~CGreatPart()
{

}
CGreatPart* CGreatPart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CGreatPart*		pPetal = new CGreatPart(pDevice);
...
HRESULT CGreatPart::SetConstantTable(void)
{
...
HRESULT CGreatPart::AddComponent(void)
{
...
HRESULT CGreatPart::Initialize(void)
{
	AddComponent();
...
void CGreatPart::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CGreatPart::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CGreatPart::SetPart(int nIdx)
{
	...
	pPrt->m_dColor	=	D3DXCOLOR(1.f, 1.f, 0.25f, pPrt->m_fLife);


	// 입자의 표현 요소
	pPrt->m_PrsW = 100.f + rand()%61;
	pPrt->m_PrsW *= 0.03f;

	pPrt->m_PrsH = 100.f + rand()%61;
	pPrt->m_PrsH *= 0.03f;
