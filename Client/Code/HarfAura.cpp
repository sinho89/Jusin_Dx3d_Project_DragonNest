...
CHarfAura::CHarfAura(LPDIRECT3DDEVICE9 pDevice)
...
CHarfAura::~CHarfAura(void)
{

}

HRESULT CHarfAura::Initialize(void)
{
...
HRESULT CHarfAura::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_HarfAura");
...
CHarfAura* CHarfAura::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHarfAura*		pGameObject = new CHarfAura(pDevice);
...
CHarfAura* CHarfAura::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHarfAura*		pGameObject = new CHarfAura(pDevice);
...
HRESULT CHarfAura::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CHarfAura::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHarfAura::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
