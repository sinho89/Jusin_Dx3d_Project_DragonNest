...
void CHit::Render(void)
{
	...
	{*/
		...
		m_pDevice->SetRenderState(D3DRS_BLENDOP, D3DBLENDOP_MAX);
...
HRESULT CHit::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_Hit");
...
CHit* CHit::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHit*		pGameObject = new CHit(pDevice);
...
CHit* CHit::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHit*		pGameObject = new CHit(pDevice);
...
HRESULT CHit::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
	...
	//Light
	D3DLIGHT9*	pLightInfo = Engine::Get_LightMgr()->GetLightInfo(4);
...
void CHit::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHit::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
