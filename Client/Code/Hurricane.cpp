...
CHurricane::CHurricane(LPDIRECT3DDEVICE9 pDevice)
...
HRESULT CHurricane::Initialize(void)
{
...
HRESULT CHurricane::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_Hurrycane");
...
CHurricane* CHurricane::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHurricane*		pGameObject = new CHurricane(pDevice);
...
CHurricane* CHurricane::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHurricane*		pGameObject = new CHurricane(pDevice);
...
HRESULT CHurricane::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CHurricane::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHurricane::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
