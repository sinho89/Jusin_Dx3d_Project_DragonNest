...
CHarfDust::CHarfDust(LPDIRECT3DDEVICE9 pDevice)
...
CHarfDust::~CHarfDust(void)
{

}

HRESULT CHarfDust::Initialize(void)
{
...
HRESULT CHarfDust::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_HarfDust");
...
CHarfDust* CHarfDust::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHarfDust*		pGameObject = new CHarfDust(pDevice);
...
CHarfDust* CHarfDust::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHarfDust*		pGameObject = new CHarfDust(pDevice);
...
HRESULT CHarfDust::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CHarfDust::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHarfDust::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
