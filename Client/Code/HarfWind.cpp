...
CHarfWind::CHarfWind(LPDIRECT3DDEVICE9 pDevice)
...
CHarfWind::~CHarfWind(void)
{

}

HRESULT CHarfWind::Initialize(void)
{
...
HRESULT CHarfWind::AddComponent(void)
{
	...
	//Mesh
	pComponent = m_pResourceMgr->CloneResource(RESOURCE_STAGE, L"Mesh_HarfWind");
...
CHarfWind* CHarfWind::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHarfWind*		pGameObject = new CHarfWind(pDevice);
...
CHarfWind* CHarfWind::Create(LPDIRECT3DDEVICE9 pDevice , D3DXVECTOR3 _vPos, float _fAngle)
{
	CHarfWind*		pGameObject = new CHarfWind(pDevice);
...
HRESULT CHarfWind::SetConstantTable(void)
{
	LPD3DXEFFECT	pEffect = m_pShader->Get_EffectHandle();
...
void CHarfWind::SetInfo(D3DXVECTOR3 _vPos, float	_fAngle)
{
	m_pInfo->m_vPos = _vPos;
	m_pInfo->m_fAngle[Engine::ANGLE_Y] = D3DXToRadian(_fAngle);
}

void CHarfWind::SetType(OBTAINTYPE _eType)
{
	m_eType = _eType;
}
